...
│           ├── login.js              # 登入 / 註冊雙 tab
│           ├── orders.js             # 我的訂單列表
│           ├── order-detail.js       # 訂單詳情 + 前往綠界付款 + 主動查詢付款
...
│       ├── login.ejs                 # 登入 / 註冊
│       ├── orders.ejs                # 訂單列表
│       ├── order-detail.ejs          # 訂單詳情，#app[data-order-id][data-payment-result]；含「前往綠界付款」與「主動向綠界查詢付款結果」按鈕
│       └── admin/
│           ├── products.ejs          # 商品管理（含 Modal）
│           └── orders.ejs            # 訂單管理（含側拉詳情）
│
├── src/
│   ├── database.js                   # 唯一的 DB 模組：初始化、建表、seed、idempotent ALTER 遷移
│   ├── middleware/
...
│   ├── services/
│   │   └── ecpay.js                  # 綠界 AIO 整合：ecpayUrlEncode、CheckMacValue、AIO 參數產生、QueryTradeInfo
│   └── routes/
│       ├── authRoutes.js             # POST /register、/login；GET /profile
│       ├── productRoutes.js          # GET /、/:id（公開）
│       ├── cartRoutes.js             # GET/POST/PATCH/DELETE；內含 dualAuth
│       ├── orderRoutes.js            # POST 建立、GET 列表、GET 詳情、PATCH /:id/pay、POST /:id/ecpay-checkout、POST /:id/ecpay-query
│       ├── paymentRoutes.js          # 公開：GET /payment/ecpay/:id 跳轉頁、/payment/return/:id 回來頁、POST /ecpay/notify
...
└── tests/                            # Vitest 測試
    ...
    ├── adminOrders.test.js
    └── ecpay.test.js                 # ecpay service 單元測試（CheckMacValue 官方測試向量 + fetch mock）
```
...
3. **`app.js`** 組裝順序（影響行為，請維持）：
   ...
   7. 掛載付款公開路由 `/`（`src/routes/paymentRoutes.js`），提供綠界跳轉頁 `/payment/ecpay/:id`、OrderResultURL 接收頁 `/payment/return/:id` 與 `/ecpay/notify`。**必須掛在頁面路由之前**，否則 `pageRoutes` 的 `/orders/:id` 會被誤匹配。
   8. 掛載頁面路由 `/`。
   9. 404 fallback：若路徑以 `/api` 開頭回 JSON；否則 render `pages/404.ejs` 套上 front layout。
   10. 最後接 `errorHandler`。
4. **`src/database.js`** 模組首次載入時執行 `initializeDatabase()`：
   ...
   - **Idempotent ALTER 遷移**：以 `PRAGMA table_info(orders)` 取得目前欄位，再對缺少的欄位 `ALTER TABLE ADD COLUMN`。目前用於補上 ECPay 整合所需的 `ecpay_trade_no` / `ecpay_tx_no` / `payment_method` / `paid_at`。新增欄位請延續這個模式。
...
| `/api/orders` | `src/routes/orderRoutes.js` | JWT 必填（整個 router 套用 `authMiddleware`） | 建立訂單、列表、詳情、模擬付款、ECPay 結帳參數、ECPay 主動查詢 |
...
| `/payment/*`、`/ecpay/notify` | `src/routes/paymentRoutes.js` | **公開（不經 authMiddleware）** | 綠界結帳跳轉頁、OrderResultURL 接收頁、Server Notify 入口 |
...
| PATCH | `/api/orders/:id/pay` | JWT | 模擬付款（body: `action: success\|fail`）—— 保留作為向下相容，前台已不使用 |
| POST | `/api/orders/:id/ecpay-checkout` | JWT | 產生綠界 AIO 付款參數（含 CheckMacValue），每次呼叫重新產生 MerchantTradeNo |
| POST | `/api/orders/:id/ecpay-query` | JWT | 主動向綠界 QueryTradeInfo/V5 查詢付款結果並冪等更新訂單 |
| POST | `/ecpay/notify` | 公開 | 綠界 Server Notify 入口（本機環境收不到；保留並回 `1\|OK`） |
| GET / POST | `/payment/return/:id` | 公開 | 綠界 OrderResultURL 回來頁，於前端觸發 `/api/orders/:id/ecpay-query` |
| GET | `/payment/ecpay/:id` | 公開（前端取 JWT） | 自動 submit form 跳轉到綠界付款頁 |
...
| `/orders/:id` | `pages/order-detail.ejs` | 帶入 `orderId` 與 `paymentResult`（query `?payment=...`）；自綠界導回時若 status 仍為 pending 會自動觸發一次主動查詢 |
| `/payment/ecpay/:id` | — | `paymentRoutes.js` server-render 一個自動跳轉頁：fetch `/api/orders/:id/ecpay-checkout` 取得 AIO 參數後 form.submit 至綠界 |
| `/payment/return/:id` | — | `paymentRoutes.js` server-render 「處理中」頁面，瀏覽器載入後立刻 POST `/api/orders/:id/ecpay-query`，依結果 redirect 回 `/orders/:id?payment=success\|failed\|pending` |
...
| `NO_ECPAY_TRADE` | 400 | 呼叫 `/api/orders/:id/ecpay-query` 時訂單尚未發起綠界付款（`ecpay_trade_no` 為 null） |
...
| `AMOUNT_MISMATCH` | 502 | 綠界 QueryTradeInfo 回傳金額與訂單金額不符（防竄改） |
...
| `ecpay_trade_no` | TEXT | NULL | 送往綠界的 `MerchantTradeNo`，每次點「前往綠界付款」會重新產生並覆寫 |
| `ecpay_tx_no` | TEXT | NULL | 綠界回傳的 `TradeNo`（綠界端交易編號），主動查詢成功後寫入 |
| `payment_method` | TEXT | NULL | 綠界回傳的 `PaymentType`，如 `Credit_CreditCard` |
| `paid_at` | TEXT | NULL | 綠界回傳的 `PaymentDate`（`yyyy/MM/dd HH:mm:ss`） |
| `created_at` | TEXT | NOT NULL DEFAULT `datetime('now')` | |

> 後 4 個欄位以 idempotent ALTER 形式於 `initializeDatabase()` 末尾加入，舊資料庫升級時不會遺失既有資料。
...
本機環境（`localhost`）無法接收綠界 Server Notify（ReturnURL 為 Server-to-Server 且綠界僅支援 port 80/443），因此付款結果確認**改由本地端主動呼叫綠界 QueryTradeInfo/V5 驗證**，而非等待 callback。
...
消費者                 Flower Life（localhost）              ECPay Stage
  ...
  │                          │   產生 MerchantTradeNo + AIO 參數 │
  │ form.submit (action=AioCheckOut/V5) ────────────────────────→│
  │                          │                                   │
  │  在綠界完成測試付款（4311-9522-2222-2222 / 3DS 1234）       │
  │  ←── ReturnURL（Server Notify）  ✗ localhost 不可達 ───────┤
  │                          │                                   │
  │ ←── 導回 OrderResultURL: /payment/return/:id ──────────────┤
  │  瀏覽器自動 POST /api/orders/:id/ecpay-query                │
  │                          │ → QueryTradeInfo/V5 + 驗章 ────→│
  │                          │ ← 解析 TradeStatus → 更新訂單   │
  │ ←── redirect /orders/:id?payment=success                    │
```

**關鍵規則**（已落實於 `src/services/ecpay.js` 與 `src/routes/orderRoutes.js`）：
- `MerchantTradeNo` 為**永久唯一**，每次點「前往綠界付款」會重新產生並覆寫 `orders.ecpay_trade_no`。
- `CheckMacValue` 使用 SHA256 + `ecpayUrlEncode`（含 toLowerCase 與 .NET 字元還原）；驗章使用 `crypto.timingSafeEqual`，禁止用 `==`。
- 訂單 status 機器：`pending → paid`（`TradeStatus=1`）／`pending → failed`（`TradeStatus=10200095`）；其餘 TradeStatus 不動，可重複查詢。
- **金額防竄改**：QueryTradeInfo 回傳的 `TradeAmt` 必須與 `orders.total_amount` 相符，否則回 502 `AMOUNT_MISMATCH`。
- 庫存在建單即扣，付款結果只更新 status，不再動庫存（避免重複扣或補回的複雜性）。
- `POST /ecpay/notify` 仍實作並回 `1|OK`，便於未來透過 ngrok / Cloudflare Tunnel 開放外網時直接生效；實際付款確認仍以主動查詢為準。

### 模擬付款（`PATCH /api/orders/:id/pay`）— 保留作向下相容

- Body：`{ "action": "success" | "fail" }`，對應 `success → paid`、`fail → failed`。
- 必須是 `pending` 才能轉換，否則 400 `INVALID_STATUS`。
- **前台已不使用**（checkout 後直接導向 `/payment/ecpay/:id`），保留是為了既有測試（`tests/orders.test.js`）與後台手動操作。
...
| 項目 | 設定 |
| --- | --- |
| 服務 | AIO（全方位金流），`ChoosePayment=Credit`（信用卡） |
| 協議 | CMV-SHA256 |
| 建單端點 | Staging `https://payment-stage.ecpay.com.tw/Cashier/AioCheckOut/V5`；Production `https://payment.ecpay.com.tw/Cashier/AioCheckOut/V5` |
| 查詢端點 | Staging `https://payment-stage.ecpay.com.tw/Cashier/QueryTradeInfo/V5`；Production `https://payment.ecpay.com.tw/Cashier/QueryTradeInfo/V5` |
| 切換環境 | `.env` 的 `ECPAY_ENV=staging \| production` |
| 介接資訊 | `.env` 的 `ECPAY_MERCHANT_ID`、`ECPAY_HASH_KEY`、`ECPAY_HASH_IV`（預設為綠界官方公開測試帳號 `3002607` / `pwFHCqoQZGmho4w6` / `EkRm7iFT261dpevs`） |
| 封裝模組 | `src/services/ecpay.js`（純 Node.js 實作，未使用官方 PHP SDK） |
| 付款確認 | 本地主動查詢（QueryTradeInfo/V5）。`TimeStamp` 3 分鐘有效期，每次呼叫前重新產生 |
| Server Notify | `POST /ecpay/notify` 已實作並回 `1\|OK`，但本機環境綠界打不到（localhost 不可達、port 3001 非 80/443） |
| 加密驗證 | `tests/ecpay.test.js` 包含官方 3 組 CheckMacValue 測試向量 + fetch mock 驗證 |

### 已知限制（與綠界官方建議的差異）

- **不支援 ATM / 超商代碼 / 條碼**：這些屬於離線付款，付款後綠界需透過二次 callback 通知，本機環境無法接收，且需排程主動查詢才能取得最終狀態。目前固定 `ChoosePayment=Credit`。
- **不實作 DoAction（請款 / 退款 / 取消）**：尚無需求，後續可在 `src/services/ecpay.js` 補上對應方法。
- **不下載對帳檔**：尚無需求。
- **本機 ReturnURL 不可達**：綠界 Server Notify 為 Server-to-Server 且僅支援 port 80/443，無法打到 `http://localhost:3001`。實務上不影響訂單狀態（因為以主動查詢為準），但綠界後台會顯示「callback 失敗」並嘗試重送 4 次。若要避免，可改用 ngrok / Cloudflare Tunnel 將 `/ecpay/notify` 暴露至公網。
